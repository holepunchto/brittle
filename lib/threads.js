...
class ThreadHandler {
  ...
  constructor({ thread, file, connection, onConfig }) {
    ...
    const { promise: initialConfig, resolve: setInitialConfig } = Promise.withResolvers()
    this.initialConfig = initialConfig
    this.setInitialConfig = setInitialConfig
...
class Threads {
  ...
  run() {
    ...
    setImmediate(async () => {
      ...
      this.threads.forEach((t) => t.thread.join())
    })
  }
  ...
  startWorkers() {
    ...
    return Array.from({ length: this.runner.jobs }, () => worker())
  }
