  ...
  constructor({ thread, file, connection, onStateUpdate }) {
    ...
    this.connection.on('data', (data) => {
      if (data.type === 'log') this.logs.push(data)
        ...
        setResult(data)
        if (data.bailout !== undefined) this.thread.terminate() // a bailed out job may never end
    ...
    connection.on('error', (error) => setError(error))
      ...
      this.isDone = true
      setResult({ bailout: 'Job exited without reporting results' }) // noop if resolved
    })
  }
...
class Threads {
  ...
  async tapOutput() {
    ...
    while (currentIndex < this.threads.length) {
      ...
      if (result.bailout !== undefined) {
        this.threads.forEach((t) => t.thread.terminate())
        this.runner.bailout(result.bailout)
        return
      }
  ...
  add(file) {
    ...
    const thread = new Thread(file, { data: { handle, isBrittleThread: true } })
