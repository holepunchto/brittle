...
class ThreadHandler {
  ...
  constructor({ thread, file, connection, onStateUpdate }) {
    ...
    connection.on('error', (error) => setError(error))
      ...
      this.isDone = true
      setResult(null) // noop if resolved
    })
  }
...
class Threads {
  ...
  async tapOutput() {
    ...
    while (currentIndex < this.threads.length) {
      ...
      if (!(await current.result)) {
        global.Bare.exitCode = 1
        this.runner.skipAll = true
        this.threads.forEach((t) => t.thread.terminate())
        this.runner.log('results', null, null, 'Job exited without reporting results')
        return
      }
