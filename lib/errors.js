...
function explain (ok, message, assert, stackStartFunction, actual, expected, top, extra) {
  ...
  const errString = err.toString !== Object.prototype.toString ? err.toString() : Error.toString.call(err)
  stackScrub(err, errString)
  ...
  if (err.stack) {
    info.stack = err.stack.slice(errString.length + 1).split('\n').map((line) => {
      let match = false
...
function stackScrub (err, errString) {
  if (err && err.stack) {
    const stack = '\n' + err.stack.slice(errString.length + 1)
    const scrubbed = parseStack({ stack }).filter(({ fileName }) => !IGNORE.includes(fileName))
