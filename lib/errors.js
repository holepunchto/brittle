...
const IGNORE =
  typeof __filename === 'string'
    ? [__filename, __filename.replace(/lib(\/|\\)errors\.js$/, 'index.js')]
    : []
...
function explain(
  ...
  if (top) {
    ...
    try {
      ...
      if (fs) {
        ...
        const point =
          Array.from({ length: column - 1 })
            .map(() => '-')
            .join('') + '^'
        const source = [
          ...split.slice(line - 2, line),
          point,
          ...split.slice(line, line + 2)
        ]
  ...
  if (
    ...
    actual?.code === 'ERR_TIMEOUT' ||
    actual?.code === 'ERR_PREMATURE_END'
  )
    delete info.stack
...
function stackScrub(err) {
  if (err && err.stack) {
    ...
    const stack = '\n' + err.stack.slice(errString.length + 1)
    const scrubbed = parseStack({ stack }).filter(
      ({ fileName }) => !IGNORE.includes(fileName)
    )
