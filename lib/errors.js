...
function explain (ok, message, assert, stackStartFunction, actual, expected, top, extra) {
  ...
  if (err.stack) {
    const errStringLen = Error.prototype.toString.call(err).length
    info.stack = err.stack.slice(errStringLen + 1).split('\n').map((line) => {
      let match = false
...
function stackScrub (err) {
  if (err && err.stack) {
    const errString = Error.prototype.toString.call(err)
