...
function explain (ok, message, assert, stackStartFunction, actual, expected, top, extra) {
  ...
  if (top) {
    ...
    try {
      try {
        if (url) at.file = url.fileURLToPath(new URL(at.file, 'file:'))
      } catch {}

      if (at.file.startsWith(cwd)) {
        at.file = at.file.replace(cwd, '.')
      }

      if (fs) {
        const code = fs.readFileSync(at.file, { encoding: 'utf-8' })
        const split = code.split(/[\n\r]/g)
        const point = Array.from({ length: at.column - 1 }).map(() => '-').join('') + '^'
        const source = [...split.slice(at.line - 2, at.line), point, ...split.slice(at.line, at.line + 2)]
